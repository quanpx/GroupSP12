...
public class CartRepository {
    ...
    public void addProductToCart(Product product) {
        ProductInCart newProduct = new ProductInCart(product.getProduct_id(), product.getProduct_name(), product.getPrice(), 1);
        ...
        return tempProduct
                .map(product -> {
                    ...
                    if (indexOfProductInCartToUpdate >= 0) {
                        productInCartList.set(indexOfProductInCartToUpdate, new ProductInCart(product_id, tempProduct.get().getProduct_name(), tempProduct.get().getPrice(), productInCart.getQuantity()));
                        return 1;
                    }
                    return 0;
                })
